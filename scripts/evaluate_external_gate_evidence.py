...
def gate_status(gate: dict[str, Any], state: dict[str, Any]) -> tuple[bool, str]:
    kind = gate.get("kind")
    ...
    if kind == "crd":
        return (False, "kubectl explain live output is not represented by smoke evidence manifest")
    if kind == "controller-resource-budget":
        return (False, "controller resource measurement evidence is not represented by smoke evidence manifest")
    if kind == "controller":
        return (False, "controller live loop/resource evidence is not represented by smoke evidence manifest")
...
def evaluate(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for gate in plan["gates"]:
        covered, reason = gate_status(gate, state)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = evaluate(load_manifest(Path(args.manifest)))
