...
| Release evidence status | DONE | `scripts/verify_release_evidence_status.py` verifies partial evidence directory status inspection plus version-scoped `--version` blocker drilldowns, persisted next-task output, file readiness, evidence-build status, queue-source visibility/origin, next-task queue consistency, runner/build/advance record consistency, and legacy prepared-record source inference |
