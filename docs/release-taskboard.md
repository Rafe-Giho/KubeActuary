...
| Live evidence directory scaffold | DONE | `scripts/prepare_live_evidence_directory.py` and `scripts/verify_live_evidence_directory_scaffold.py` verify local reports/raw/supplemental/.kubeactuary scaffold generation plus prepared-queue-sourced and probe-aware next-task artifacts, schemas `kube-actuary.environment-probe.v1` and `kube-actuary.environment-blockers.v1`, and `--skip-complete-evidence` advancement |
