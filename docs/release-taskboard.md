...
| Release evidence status | DONE | `scripts/verify_release_evidence_status.py` verifies partial evidence directory status inspection plus persisted next-task output, file readiness, queue-source visibility, and legacy prepared-record source inference |
