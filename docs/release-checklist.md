...
- version worklist verifies schema `kube-actuary.version-worklist.v1`,
  ...
  worklist drilldowns, latest advance/runner status, latest run artifact paths,
  latest aggregate and per-version diff summaries, next local loop commands,
  latest next-task evidence file details, latest environment probe failures, and records
