...
- version worklist check prints `version-worklist: passed` and exercises
  ...
  iteration history queue-source, blocker drilldown, latest artifact paths,
  latest diff summary, and next-command preservation across text, JSON,
  Markdown, recorded status reports, and latest probe failures,
...
- release evidence status check prints `release-evidence-status: passed` and
  ...
  text/Markdown next-command and next-task detail output, runner/advance record
