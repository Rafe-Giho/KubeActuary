...
- live evidence directory scaffold generation for repeated evidence capture,
  ...
  `--skip-complete-evidence` advancement, failed-probe retry deferral,
...
- release evidence status inspection for partial and complete evidence
  ...
  worklist drilldowns, version-scoped coverage totals, missing gates, and blocker drilldown commands, and
...
- release evidence status check prints `release-evidence-status: passed` and
  ...
  build, next-task-run status, next-unblock runner status, next-unblock retry command metadata,
  selected next-unblock next-step output, guarded next-unblock retry metadata,
  version history next-unblock action/run/retry preservation, environment metadata, advance status,
