...
NEXT_UNBLOCK_ACTION_RUNNER = ROOT / "scripts" / "verify_next_unblock_action_runner.py"
...
VERSION_ITERATION_ADVANCE = ROOT / "scripts" / "verify_version_iteration_advance.py"
...
class ReleaseToolTests(unittest.TestCase):
    ...
    def test_verify_release_progress(self):
        result = subprocess.run(
            [sys.executable, str(RELEASE_PROGRESS)],
            cwd=ROOT,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    def test_verify_version_unblock_plan(self):
        result = subprocess.run(
            [sys.executable, str(VERSION_UNBLOCK_PLAN)],
            cwd=ROOT,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    def test_verify_next_unblock_action_runner(self):
        result = subprocess.run(
            [sys.executable, str(NEXT_UNBLOCK_ACTION_RUNNER)],
            cwd=ROOT,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("next-unblock-action-runner: passed", result.stdout)
    ...
    def test_verify_version_iteration_advance(self):
        result = subprocess.run(
            [sys.executable, str(VERSION_ITERATION_ADVANCE)],
            cwd=ROOT,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("version-iteration-advance: passed", result.stdout)
        self.assertIn("history-runs: 2", result.stdout)
