...
v0.9.5는 pre-GA, local-complete 릴리스입니다. 현재 저장소 범위를 기준으로
CLI, capsule schema, CRD seed, 저부하 controller contract, optional packaging
asset, policy evidence adapter, safe agent integration surface, admission
prototype이 포함되어 있습니다.

하지만 v0.9.5는 아직 1.0.0 production claim이 아닙니다. 1.0.0까지 남은
작업은 승인된 Kubernetes와 provider 환경에서 live evidence를 캡처하는
것입니다. 여기에는 cluster smoke run, Helm/Krew install proof,
managed-provider check, resource-budget evidence가 포함됩니다. 필요한 도구나
네트워크 접근이 없는 환경에서는 이 항목들이 의도적으로 blocked 상태로
남습니다.
...
- optional Helm, Kustomize, Krew, SBOM, provenance, air-gap packaging asset
- 일반 Kubernetes policy tool을 위한 policy evidence adapter support
- safe MCP/agent integration surface
- optional admission prototype asset
...
examples/             capsule, manifest, CRD, agent integration 예제
